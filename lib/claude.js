...
- romantic: rooftop restaurant, fine dining, lounge restaurant, upscale café, restaurant with ambiance
- casual: café, coffee shop, food court, restaurant, park, street food market
- adventurous: escape room, bowling alley, arcade, go-karting, live music venue, comedy club

SEARCH GUIDANCE:
- Use a search radius of 3000–5000 meters (not the default 1500) — venue density in Indian cities is uneven
- Prefer broad keywords like "restaurant" over niche ones like "wine bar" which are rare outside major Western cities
- If a keyword search returns no results, try a broader term (e.g. "restaurant" instead of "fine dining")
...
export function parseVenueResponse(responseText) {
  ...
  if (jsonEnd === -1) throw new Error('Failed to parse venues: no closing brace found')
  const cleanJson = jsonStr.slice(0, jsonEnd + 1)
  let parsed
  try {
